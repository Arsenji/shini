#!/usr/bin/env python3
"""Gentle VENTI 1704 install — no silhouette cutout, no heavy inpaint."""
...
def optional_watermark_pass(img: np.ndarray) -> tuple[np.ndarray, int]:
    """One light TELEA pass (r=2) on obvious faint diagonal gray streaks over black areas."""
    ...
    count = int(wm.sum())
    # Conservative: skip unless clearly watermark-like (enough pixels, not too many).
    if count < 80 or count > 8000:
        return img, 0
...
def fit_canvas(arr: np.ndarray) -> Image.Image:
    """Resize full rectangular image (no cutout), center on white 900x1200."""
    h, w = arr.shape[:2]
...
def clean_canvas_edge_artifacts(img: np.ndarray, band: int = 5) -> tuple[np.ndarray, int]:
    """Paint pure white on thin dark frame lines in outer band of canvas only."""
    h, w = img.shape[:2]
    out = img.copy()
    maxc = out.max(axis=2)
    ...
    # Thin dark lines on white: low max channel, bright neighbors outside dark core.
...
def save_outputs(canvas: Image.Image) -> None:
    ...
    canvas.save(TMP / "venti1704-gentle.jpg", "JPEG", quality=90, optimize=True)
...
def main() -> None:
    ...
    mask = content_mask(img)
    print(f"barrel_on_left={barrel_on_left(mask)} (True=face-right, no flip needed)")
    ...
    img, wm_px = optional_watermark_pass(img)
    print(f"watermark_pixels_inpainted={wm_px} (0=skipped or none)")
    ...
    print("saved public/dist/backend + .tmp-disk/venti1704-gentle.jpg")
