#!/usr/bin/env python3
"""Install shared rim-tape / flipper photo for all ободные ленты (flipper-rim-tape.jpg)."""
...
PUBLIC = ROOT / "public/tires"
DIST = ROOT / "dist/tires"
BACKEND = ROOT / "backend/static/tires"
TMP = ROOT / ".tmp-disk"
STEM = "flipper-rim-tape"
SOURCE = Path(
    "/Users/arsenijberdnikov/.cursor/projects/Users-arsenijberdnikov-Documents/assets/"
    "image-63c65c76-4b88-4c1a-aa3f-88c71d684f4d.png"
)
TW, TH = 900, 1200
FILL = 0.84
...
def crop_to_content(arr: np.ndarray, pad: int = 8) -> np.ndarray:
...
def save_outputs(canvas: Image.Image) -> None:
    ...
    canvas.save(TMP / "flipper-rim-tape-new.jpg", "JPEG", quality=90, optimize=True)
...
def main() -> None:
    ...
    print("keeping source orientation (no flip)")

    canvas = fit_canvas(cropped)
    ...
    print("saved + .tmp-disk/flipper-rim-tape-new.jpg")
